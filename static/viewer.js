(() => {
  ...
  function renderDarkness() {
    ...
    const maskId = "viewerDarknessMask";
    const terrainMaskId = "viewerTerrainLosMask";
    const nightMaskId = "viewerNightvisionMask";
    const grayFilterId = "viewerNightvisionGray";
    const exploredInvertFilterId = "viewerExploredInvert";
    const unexploredMaskId = "viewerUnexploredMask";
    const exploredDimMaskId = "viewerExploredDimMask";
  ...
  function renderVisibleDoors() {
    ...
    const height = content.clientHeight;
    const ns = "http://www.w3.org/2000/svg";
    const clipId = "viewerPublicDoorVisionClip";
    const gridFraction = Math.max(0.01, Number(state.grid_size) || 0.05);
  ...
  function renderTokens() {
    ...
    for (const token of state.tokens || []) {
      const element = document.createElement("div");
      element.className = "map-token readonly-token";
  ...
  async function load() {
    try {
      const response = await fetch(scopedUrl("/api/state"), { cache: "no-store" });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      apply(await response.json());
    } catch (error) {
      console.error("Map state load failed", error);
    }
  }
